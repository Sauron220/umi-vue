<template>
    <div class="wrap notice-list">
        <PageNav :message="goBacks()"></PageNav>
        <div class="mui-content">
            <v-scroll :on-refresh="onRefresh" :on-infinite="onInfinite" ref="hasBox">
                <a class="item" :href="'/notice/'+item.newsId" v-for="item in noticeList">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goBacks(){
        if(this.$route.query && this.$route.query.ind == 'f'){
          return '/'
        } else {
          return '/profile'
        }
